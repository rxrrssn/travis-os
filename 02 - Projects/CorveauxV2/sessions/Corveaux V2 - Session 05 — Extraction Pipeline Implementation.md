...
- [x] Fixed Stop hook — changed `additionalContext` to `systemMessage` to break infinite loop (additionalContext injects into model context → model responds → triggers another Stop → repeat)
