...
- [x] Fix P2002 promoter bug — partial unique index on `canonical_key WHERE valid_to IS NULL` via raw SQL migration (Session 07)
- [x] Fix extractor type leakage — filter platform-level types (`environment`, `domain`, `repository`, `credential`, `milestone`) out of the system prompt extraction registry (Session 07)
- [x] Investigate SLCC catalog rendering — Acalog pages ARE static HTML, not JS-rendered. Root cause was output token limit, not rendering. Individual program pages (`preview_program.php`) are the extraction target. (Session 07)
- [x] Once catalog fetch is solved, re-run smoke test targeting programs and courses — pilot complete: 9 pages, 6 programs, 0.965 avg confidence (Session 08)
- [x] Verify Entra ID auth end-to-end in browser — Tenant Zero auth validated in [[Corveaux V2 - Session 11 — Tenant Zero Auth Validation]]
